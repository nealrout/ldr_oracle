BEGIN
    FOR c IN (SELECT COUNT(*) cnt
        ...
        IF c.cnt > 0 THEN
            EXECUTE IMMEDIATE 'ALTER TABLE LOAD_LOG_DETAIL DROP CONSTRAINT FK_LOAD_LOG_DETAIL_LOAD_LOG_ID';
            DBMS_OUTPUT.PUT_LINE('Foreign key constraint FK_LOAD_LOG_DETAIL_LOAD_LOG_ID has been dropped.');
        ELSE
            DBMS_OUTPUT.PUT_LINE('Foreign key constraint FK_LOAD_LOG_DETAIL_LOAD_LOG_ID does not exist.');
        END IF;
    END LOOP;
    ...
    FOR c IN (SELECT COUNT(*) cnt
        ...
        IF c.cnt > 0 THEN
            EXECUTE IMMEDIATE 'ALTER TABLE LOAD_CONFIG DROP CONSTRAINT FK_LOAD_CONFIG_LOAD_TYPE_LOAD_TYPE_CODE';
            DBMS_OUTPUT.PUT_LINE('Foreign key constraint FK_LOAD_CONFIG_LOAD_TYPE_LOAD_TYPE_CODE has been dropped.');
        ELSE
            DBMS_OUTPUT.PUT_LINE('Foreign key constraint FK_LOAD_CONFIG_LOAD_TYPE_LOAD_TYPE_CODE does not exist.');
        END IF;
    END LOOP;
