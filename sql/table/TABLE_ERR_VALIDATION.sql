...
BEGIN
    ...
    SELECT COUNT(*) INTO v_count FROM user_indexes WHERE index_name = 'IDX_ERR_VALIDATION_CREATE_TS';
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX IDX_ERR_VALIDATION_CREATE_TS ON ERR_VALIDATION(CREATE_TS)';
    END IF;
    
    SELECT COUNT(*) INTO v_count FROM user_indexes WHERE index_name = 'IDX_ERR_VALIDATION_LOAD_LOG_DETAIL_ID_UNIQUE_IDENTIFIER_VALUE';
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE INDEX IDX_ERR_VALIDATION_LOAD_LOG_DETAIL_ID_UNIQUE_IDENTIFIER_VALUE ON ERR_VALIDATION(LOAD_LOG_DETAIL_ID, UNIQUE_IDENTIFIER_VALUE)';
    END IF;
END;